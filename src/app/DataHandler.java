package app;

// Оброблення вхідних даних різного типу
public class DataHandler {

    // Узагальнений нестатичний метод, який має параметр
    // як масив узагальненого типу та повертає String значення
    // обробленого масиву
    public <T> String handleData(T[] items) {
        StringBuilder sb = new StringBuilder();
        int count = 0;
        // Цикл обробки масиву узагальненого типу
        for (T item : items) {
            count++;
            sb.append(String.format("(%d) %s ", count, item.toString()));
        }
        return sb.toString();
    }

    // Метод для обробки щоденних результатів
    public String handleDailyData(String[] productNames, Double[] salesAmounts) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < productNames.length; i++) {
            sb.append(String.format("Day %d: Best-selling product: %s, Total sales: EUR %.2f%n",
                    i + 1, productNames[i], salesAmounts[i]));
        }
        return sb.toString();
    }
}